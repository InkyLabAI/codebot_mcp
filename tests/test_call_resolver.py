...
"""
import pytest
from codebot_mcp.code_parsing.code_parser import CodeParser
...
'''
        ...
        child_method = next(f for f in resolved if f.name == 'do_something' and f.class_name == 'Child')
...
class TestPackageExportsResolution:
    ...
    def test_package_export_resolution(self):
        ...
        functions = [
            create_function_chunk(
                ...
                file_path='pkg/callbacks/base.py'
            ),
            create_function_chunk(
                name='caller',
                file_path='pkg/trainer.py',
                calls=['callbacks.get_default_callbacks']
            )
        ]
        classes = []
    ...
    def test_package_export_with_init_in_path(self):
        ...
        functions = [
            create_function_chunk(
                name='helper_func',
                file_path='utils/helpers/base.py'
            ),
            create_function_chunk(
                name='caller',
                file_path='main.py',
                calls=['helpers.helper_func']
            )
        ]
        classes = []
        ...
        package_exports = {
            'utils.helpers.helper_func': 'utils.helpers.base.helper_func'
        }
...
'''
        ...
        base_init = next(f for f in resolved if f.name == '__init__' and f.class_name == 'BaseDataset')
...
'''
        ...
        yolo_init = next(f for f in resolved if f.name == '__init__' and f.class_name == 'YOLODataset')
        # super().__init__() edge
...
'''
        ...
        child_a_init = next(f for f in resolved if f.name == '__init__' and f.class_name == 'ChildA')
        assert 'module.ChildA.process' in child_a_init.calls
        assert 'module.ChildB.process' not in child_a_init.calls

        child_b_init = next(f for f in resolved if f.name == '__init__' and f.class_name == 'ChildB')
...
class TestExtractClassFromGenericType:
    ...
    def test_generator_first_param(self):
        assert CallResolver._extract_class_from_generic_type("Generator[Yield, Send, Return]") == "Yield"
...
class TestReturnTypeResolution:
    ...
    def test_direct_chained_call_resolves_method(self):
        ...
        functions = [
            create_function_chunk(
                'get_config', 'pkg/factory.py',
                class_name='Factory',
                return_type='Config'
            ),
            create_function_chunk(
                'validate', 'pkg/config.py',
                class_name='Config'
            ),
            create_function_chunk(
                'caller', 'pkg/main.py',
    ...
    def test_var_types_takes_priority_over_var_sources(self):
        ...
        functions = [
            create_function_chunk(
                'method', 'pkg/module.py',
                class_name='TypedClass'
            ),
            create_function_chunk(
                'method', 'pkg/other.py',

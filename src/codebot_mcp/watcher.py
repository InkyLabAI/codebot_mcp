...
from codebot_mcp.incremental import process_changes
from codebot_mcp.utils.gitignore_utils import load_gitignore, is_file_ignored
...
async def watch_repo(repo_path: str, repo_id: str, indexing_lock: asyncio.Lock) -> None:
    ...
    logger.info(
        "Watching %s for .py changes (debounce=%.1fs)", repo_path, DEBOUNCE_SECONDS
    )
        ...
        async for changes in awatch(repo_path, force_polling=True, poll_delay_ms=1000):
            ...
            for change_type, path in changes:
                ...
                if (
                    ...
                    pending[path] = (
                        "deleted" if change_type == Change.deleted else "changed"
                    )

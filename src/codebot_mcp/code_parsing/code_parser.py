...
class CodeParser:
    ...
    def _init_queries(self):
        self.function_query = Query(self.language, """
            (function_definition
    ...
    def _extract_decorator_call(self, decorator_text: str) -> str:
        """
        ...
        builtin_decorators = {'staticmethod', 'classmethod', 'property',
                             'abstractmethod', 'dataclass', 'contextmanager',
    ...
    def extract_calls_from_function_body(self, body_node, var_types=None, param_names=None):
        ...
        def walk_node_for_calls(node):
            ...
            if node.type == 'call':
                ...
                func_refs = self._extract_function_references_from_call(node, var_types, param_names)
    ...
    def _is_method_reference_context(self, attr_node):
        ...
        if parent.type == 'augmented_assignment' and parent.children and parent.children[0] == attr_node:
            return False
    ...
    def _get_first_positional_arg(self, arg_list_node) -> str:
        """
    ...
    def _detect_module_forward_call(self, call_info: str, var_types: dict) -> str:
        """
        ...
        if len(parts) == 2 and parts[0] == 'self':
            ...
            method_prefixes = (
                ...
                'validate_', 'process_', 'handle_', 'create_',
                'build_', 'init_', 'load_', 'save_', 'update_',
                'compute_', 'calculate_', 'run_', 'do_', 'make_',
                'add_', 'remove_', 'delete_', 'check_', 'parse_',
                'to_', 'from_', 'as_', 'with_',
                '_',  # Private methods
            )
            ...
            method_names = {
                'forward', 'backward', 'train', 'eval', 'parameters',
                'state_dict', 'load_state_dict', 'zero_grad', 'step',
                'call', 'apply', 'register', 'reset', 'clear', 'close',
            }
    ...
    def _extract_func_ref_from_arg(self, arg_node, var_types=None, param_names=None):
        ...
        if arg_node.type == 'identifier':
            ...
            callback_keywords = {'key', 'target', 'func', 'function', 'callback',
                               'handler', 'processor', 'factory', 'side_effect',
    ...
    def _is_likely_function_reference(self, name: str) -> bool:
        ...
        non_function_names = {
            ...
            'self', 'cls', 'args', 'kwargs', 'data', 'items', 'result',
            'value', 'values', 'key', 'keys', 'item', 'x', 'y', 'z',
            'i', 'j', 'k', 'n', 'm', 'a', 'b', 'c', 'd', 'e', 'f',
            # Common literals/builtins used as args
            'None', 'True', 'False', 'str', 'int', 'float', 'list',
            'dict', 'set', 'tuple', 'bytes', 'bool', 'type', 'object',
            # Common arg names
            'path', 'file', 'name', 'msg', 'message', 'text', 'string',
            'config', 'options', 'settings', 'params', 'kwargs', 'args',
        }
    ...
    def _extract_classes_from_captures(self, captures_dict, relative_path):
        ...
        super_classes = []
        if super_class_node:
            super_classes = [child.text.decode("utf-8") for child in super_class_node.named_children]
    ...
    def _resolve_relative_import(self, relative_module_text: str, current_file_path: str, all_modules: dict) -> str:
        """
        ...
        if relative_module_name:
            absolute_module = ".".join(parent_parts) + "." + relative_module_name if parent_parts else relative_module_name
        else:
    ...
    def _extract_imports_from_captures(self, captures_dict, all_modules, relative_path=None):
        ...
        elif 'import_from.module' in captures_dict and 'import_from.aliased.name' in captures_dict \
            and 'import_from.aliased.alias' in captures_dict:
            k = captures_dict['import_from.aliased.alias'][0].text.decode("utf-8")
            v = captures_dict['import_from.module'][0].text.decode("utf-8") + "."\
            + captures_dict['import_from.aliased.name'][0].text.decode("utf-8")

        # Handle relative imports (e.g., from .utils import func)
        elif 'import_from.relative_module' in captures_dict and 'import_from.relative_name' in captures_dict:
            ...
            if relative_path:
                absolute_module = self._resolve_relative_import(relative_module, relative_path, all_modules)
                if absolute_module:
                    v = absolute_module + "." + k

        elif 'import_from.relative_module' in captures_dict and 'import_from.relative_aliased.name' in captures_dict \
            ...
            relative_module = captures_dict['import_from.relative_module'][0].text.decode("utf-8")
            original_name = captures_dict['import_from.relative_aliased.name'][0].text.decode("utf-8")

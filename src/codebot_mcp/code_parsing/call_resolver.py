...
class CallResolver:
    ...
    def __init__(
        ...
        package_exports: Dict[str, str] = None
    ):
        """
    ...
    def _extract_class_from_generic_type(type_str: str):
        ...
        skip_wrappers = {'Dict', 'Mapping', 'MutableMapping', 'DefaultDict',
                         'OrderedDict', 'Tuple', 'Set', 'FrozenSet'}
    ...
    def _resolve_single_call(self, call: str, function, _resolving_source=False) -> List[str]:
        ...
        # Handle self calls
        if first_part == "self":
            resolved = self._resolve_self_call(parts, function)
        ...
        if len(parts) >= 2 and first_part not in self.imports:
            resolved = self._resolve_by_method_name_fallback(parts)
            if resolved:
                return resolved
    ...
    def _find_init_for_class(self, class_name: str) -> str:
        """
    ...
    def _compute_mro(self, class_name: str) -> List[str]:
        ...
        parents = [self._clean_superclass_name(s) for s in class_obj.superclasses]

        # C3 linearization: merge([class], parent_mros..., [parents])
        parent_mros = [self._compute_mro(p) for p in parents]

        try:
            mro = self._c3_merge([[class_name]] + parent_mros + [parents])
        except ValueError:
    ...
    def _find_method_in_mro(self, class_name: str, method_name: str) -> str:
        """
    ...
    def _clean_superclass_name(self, superclass: str) -> str:
        """
    ...
    def _resolve_superclass(self, superclass_name: str, child_class) -> str:
        """
    ...
    def _resolve_self_call(self, parts: List[str], function) -> str:
        """
        Resolve self.method calls following MRO.
        ...
        if len(parts) == 1:
            ...
            # "self.method" - find method following MRO
            method_name = parts[1]
            ...
            # Only return if method name is a known internal
            if method_name in self.all_internals:
                return f"{module_path}.{function.class_name}.{method_name}"

        return None

    def _resolve_cls_call(self, parts: List[str], function) -> str:
        """
    ...
    def _resolve_super_call(self, parts: List[str], function) -> List[str]:
        ...
        for method in parent_self_methods:
            ...
            parent_resolved = self._find_method_in_mro(
                parent_class.rsplit(".", 1)[-1], method
            )
    ...
    def _resolve_imported_call(self, parts: List[str], import_alias: str) -> str:
        """
        ...
        resolved = self._resolve_through_package_exports(resolved)

        return resolved

    def _resolve_through_package_exports(self, path: str) -> str:
        """
    ...
    def _resolve_internal_call(self, parts: List[str], function) -> str:
        """
    ...
    def _resolve_by_method_name_fallback(self, parts: List[str]) -> List[str]:
        ...
        builtin_methods = {
            ...
            'count', 'sort', 'reverse', 'copy', 'get', 'set', 'keys', 'values',
            'items', 'update', 'format', 'split', 'join', 'strip', 'replace',
            'find', 'startswith', 'endswith', 'lower', 'upper', 'encode', 'decode',
            'read', 'write', 'close', 'open', 'flush', 'seek', 'tell',
            '__init__', '__call__', '__str__', '__repr__', '__len__', '__iter__',

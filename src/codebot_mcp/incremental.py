...
import numpy as np
from sqlalchemy import select, delete, cast, Text
from sqlalchemy.ext.asyncio import AsyncSession

from codebot_mcp.config import settings
from codebot_mcp.db import get_session_maker, Function, FunctionEmbedding, Class, FileState
...
def _parse_and_resolve(repo_path: str, abs_file_path: str) -> tuple:
    ...
    """
    from codebot_mcp.setup import build_modules_dict, build_package_exports
...
async def _clean_reverse_edges(
    session: AsyncSession, repo_id: str, function_id: str
) -> None:
...
async def process_changed_file(
    ...
    try:
        functions_new, classes_new, module_docstring, class_docstrings = (
            await asyncio.to_thread(_parse_and_resolve, repo_path, abs_file_path)
        )
    ...
    existing_funcs: dict[str, Function] = {
        f.function_id: f for f in result.scalars().all()
    }
    ...
    existing_classes: dict[str, Class] = {
        c.class_id: c for c in result.scalars().all()
    }
    ...
    for fid in to_delete:
        ...
        await session.delete(existing_funcs[fid])  # FunctionEmbedding cascades

    # 4b. Collect functions that need (re-)embedding
    funcs_to_embed: list[tuple] = []  # (FunctionChunk, db_uuid: str)
    ...
    for fid in to_add:
        ...
        db_func = Function(
            ...
            calls=chunk.calls,
            search_vector=build_search_text({
                "name": chunk.name,
                "file_path": chunk.file_path,
                "class_name": chunk.class_name,
        ...
        session.add(db_func)
        funcs_to_embed.append((chunk, new_uuid))
    ...
    for fid in to_check:
        ...
        if db_func.code == chunk.code:
            continue  # Body unchanged — no re-embed needed
        ...
        db_func.parameters = [p.to_dict() for p in chunk.parameters] if chunk.parameters else None
        db_func.decorators = chunk.decorators
        db_func.return_type = chunk.return_type
        db_func.search_vector = build_search_text({
            "name": chunk.name,
            "file_path": chunk.file_path,
            "class_name": chunk.class_name,
            "parameters": [p.to_dict() for p in chunk.parameters] if chunk.parameters else [],
        ...
        # Delete old embedding so a fresh one is inserted
        del_emb = delete(FunctionEmbedding).where(
            FunctionEmbedding.function_id == db_func.id
        )
        await session.execute(del_emb)
        funcs_to_embed.append((chunk, db_func.id))
    ...
    # 6. Embed added/modified functions in a thread (blocking API call)
    if funcs_to_embed:
        func_data_list = [
            {
                "name": chunk.name,
                ...
                "docstring": chunk.docstring,
                "module_docstring": module_docstring,
                "class_docstring": class_docstrings.get(chunk.class_name) if chunk.class_name else None,
                "code": chunk.code,
                "parameters": [p.to_dict() for p in chunk.parameters] if chunk.parameters else None,
                "return_type": chunk.return_type,
            }
            for chunk, _ in funcs_to_embed
        ]
        texts = [embedding_service.prepare_function_text(d) for d in func_data_list]
        embeddings = await asyncio.to_thread(_embed_texts, texts)

        for (_, func_db_id), emb in zip(funcs_to_embed, embeddings):
            emb_array = np.array(emb, dtype=np.float32)
            session.add(FunctionEmbedding(
                id=str(uuid.uuid4()),
                function_id=func_db_id,
    ...
    logger.info(
        ...
        rel_path, len(to_add), modified_count, len(to_delete),
    )
...
async def process_deleted_file(
    ...
    # Clean reverse edges and delete functions (FunctionEmbedding cascades)
    stmt = select(Function).where(
        Function.repository_id == repo_id,
        Function.file_path == rel_path,
    )
...
async def startup_catchup(repo_id: str, repo_path: str) -> None:
    ...
    async with session_maker() as session:
        ...
        result = await session.execute(stmt)
        known: dict[str, str] = {
            fs.file_path: fs.content_hash for fs in result.scalars().all()
        }

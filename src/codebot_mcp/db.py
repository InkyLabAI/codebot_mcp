"""SQLite database layer using aiosqlite + SQLAlchemy async."""

import uuid
from datetime import datetime
from typing import Optional

from sqlalchemy import String, Integer, DateTime, Text, Float, JSON, ForeignKey, LargeBinary, event
...
class Repository(Base):
    ...
    functions: Mapped[list["Function"]] = relationship(
        "Function", back_populates="repository", cascade="all, delete-orphan",
    )
    classes: Mapped[list["Class"]] = relationship(
        "Class", back_populates="repository", cascade="all, delete-orphan",
    )
...
class Function(Base):
    ...
    # BM25 search text (pre-built, tokenized in Python)
    ...
    embedding: Mapped[Optional["FunctionEmbedding"]] = relationship(
        "FunctionEmbedding", back_populates="function", uselist=False, cascade="all, delete-orphan",
    )
...
class FunctionEmbedding(Base):
    ...
    function_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("functions.id"), nullable=False, unique=True, index=True,
    )
    # Store embedding as raw bytes (numpy .tobytes())
    embedding_blob: Mapped[bytes] = mapped_column(LargeBinary, nullable=False)
    model_name: Mapped[str] = mapped_column(String(255), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)

    function: Mapped["Function"] = relationship("Function", back_populates="embedding")
...
def get_db_path(repo_path: str) -> str:
    ...
    import os
    return os.path.join(repo_path, ".codebot.db")
